...
import com.waytoodanny.timetable.TimetableAlgorithm;
...
@Component
@Slf4j
public class GeneticTimetableAlgorithm implements TimetableAlgorithm {
...
//    while (nextGen.highestFitnessValue() != 1000) {
  ...
  private List<EvaluatedChromosome> eliteChromosomes(Population p) {
    return p.stream()
        ...
        .limit(geneticProperties.getEliteChromosomesNumber())
        .collect(toList());
  }
}
